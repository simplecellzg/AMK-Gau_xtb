...
for j in `ls -d ../sub_system_all*`
do
for i in `ls -d ${j}/llcaculation/assoc*/`
do 
    rows=0
    basename=`basename $i`
    ...
    cp ${cwd}/amk_template.dat ${workdir}/amk.dat
    sed -i '2c molecule  assoc' ${workdir}/assoc.dat
    #cp ${workdir}/assoc.dat ${workdir}/amk.dat
    screen_imagin=`awk '/imagmin/{print $2}' ${workdir}/amk.dat`
    cd $i/tsdirLL*/
    if [ -f tslist ]; then
        ...
        do 
            ...
            if [ ${image} -gt ${screen_imagin} ]; then
             if [ ! -z ${ookk} ]; then
             ...
             #  ts_xyz=`basename ${cwd}/${workdir}/hl_orca/${name_combine}_ts.xyz`
              ts_xyz=`basename ${cwd}/${workdir}/hl_orca/${name_combine}_ts_NEB-TS.xyz`
               sed 's/carga/'$charge'/;s/mult/'$mult'/;s/llts_for_hlts.xyz/'$ts_xyz'/' ${code_path}/optTS_HL_TS_orca >${cwd}/${workdir}/hl_orca/${name_combine}.inp
             ...
             echo "${cwd}/${workdir}/tsdirLL_assoc/${name_combine} imagin less than ${screen_imagin}"
             fi
        done
        echo ${basename}
        cd $cwd 
...
sed 's/amk_mol/assoc/' ${cwd}/amk_template.dat >${cwd}/${workdir}/amk.dat
#cp ${cwd}/amk_template.dat ${cwd}/${workdir}/amk.dat
cd ${cwd}/${workdir}
