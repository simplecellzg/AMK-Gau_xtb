...
##
    ...
    do 
       ...
       name=ts${i}_${ip}_${gap_num_re}_${gap_num_pr}
...
#construct g09 input file
          ...
          #level=ll
          #head
          awk '{print $0}' ${code_path}/hl_input_qst3_xtb >ts_opt/${name}_ts.dat
          #reactant
          echo "" >>ts_opt/${name}_ts.dat
          echo "reactant" >>ts_opt/${name}_ts.dat
          echo "" >>ts_opt/${name}_ts.dat
          echo "${charge} ${mult}" >>ts_opt/${name}_ts.dat
          awk '{print $0}' ${mtd_dir}/reactant.xyz >>ts_opt/${name}_ts.dat 
          #product
          echo "" >>ts_opt/${name}_ts.dat
          echo "product" >>ts_opt/${name}_ts.dat
          echo "" >>ts_opt/${name}_ts.dat
          echo "${charge} ${mult}" >>ts_opt/${name}_ts.dat
          awk '{print $0}' ${mtd_dir}/product.xyz >>ts_opt/${name}_ts.dat 
          #ts
          echo "" >>ts_opt/${name}_ts.dat
          echo "ts" >>ts_opt/${name}_ts.dat
          echo "" >>ts_opt/${name}_ts.dat
          echo "${charge} ${mult}" >>ts_opt/${name}_ts.dat
          awk '{print $0}' ${mtd_dir}/ts_qst3.xyz >>ts_opt/${name}_ts.dat 
          echo -e "\n\n" >>ts_opt/${name}_ts.dat
          # freq
          awk '{print $0}' ${code_path}/hl_input_freq_xtb >ts_opt/${name}_freq.dat
          # irc
          awk '{print $0}' ${code_path}/hl_irc_xtb >ts_opt/${name}_irc.dat
         ...
         #  echo -e "$freqg09\n\n" > ts_opt/${name}_freq.dat
          ...
          # qst3 get ts
          g09 <ts_opt/${name}_ts.dat >ts_opt/${name}_ts.log 2>/dev/null && gauss_line=$(echo $LINENO)
          # get freq
          g09 <ts_opt/${name}_freq.dat >ts_opt/${name}_freq.log 2>/dev/null
          
          ts_file=ts_opt/${name}_ts.log
          freq_file=ts_opt/${name}_freq.log
          chk_file=ts_opt/${name}_ts_freq.chk
          irc_file=ts_opt/${name}_irc.log
          cp mol.chk ${chk_file}
...
#         ok=$(awk 'BEGIN{ok=0};/Frequencies/{++nfreq;if($3<0 && $4>0 && nfreq==1) ok=1};END{print ok}' $file)
   ...
   #         g09 <ts_opt/${name}_irc.dat >${irc_file} 2>/dev/null
       ...
       fe="$(mopac_freq_ts.sh $freq_file $prog)"
